...
class DatuBase(object):
    ...
    def erabiltzaieakPrinteatu(self, punt):
        con = sqlite3.connect("datubasea.db")
        c = con.cursor()
    ...
    def gordePartida(self,tab):

        if tab.zailtasun == 1:
            errazaGorde = tab
            artxiboBinario=open("errazaGorde","wb")
            ...
            ertainaGorde = tab
            artxiboBinario = open("ertainaGorde", "wb")
            ...
            zailaGorde = tab
            artxiboBinario = open("zailaGorde", "wb")
    ...
    def kargatuPartida(self,zailtasuna):
        if zailtasuna == 1:

            if path.exists("errazaGorde"):
                artxibo=open("errazaGorde", "rb")
            ...
            if path.exists("ertainaGorde"):
                artxibo = open("ertainaGorde", "rb")
                tabGordeta = pickle.load(artxibo)
                return tabGordeta
            ...
            if path.exists("zailaGorde"):
                artxibo = open("zailaGorde", "rb")
                tabGordeta = pickle.load(artxibo)
                return tabGordeta
