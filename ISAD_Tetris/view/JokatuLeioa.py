...
class JokatuLeioa(object):
    ...
    def __init__(self,x, erab):
        ...
        if (erab == 'Admin'):
            ...
            admin.grid(row=5, column=0)

        pertso = tk.Button(self.window, text="Pertsonalizazioa",command=pertsoLeioa)
        pertso.grid(row=6, column=0)

        if x==1:
            canvas = TableroaPanela(1,(15,20),amaitu,button,gorde,kargatu,erab,master=self.window, puntuazioalabel=puntuazioa)
            button.configure(command=canvas.jolastu)
            canvas.grid(row=7, column=0, sticky=W)
        elif x==2:
            canvas = TableroaPanela(2,(10,20),amaitu,button,gorde,kargatu,erab,master=self.window, puntuazioalabel=puntuazioa)
            button.configure(command=canvas.jolastu)
            canvas.grid(row=7, column=0, sticky=W)
        elif x == 3:
            canvas = TableroaPanela(3,(10,15),amaitu,button,gorde,kargatu,erab,master=self.window, puntuazioalabel=puntuazioa)
            button.configure(command=canvas.jolastu)
            canvas.grid(row=7, column=0, sticky=W)
...
class TableroaPanela(tk.Frame):
    def __init__(self,zailtasun, tamaina,amaitu,button,gorde,kargatu,erab,gelazka_tamaina=20, puntuazioalabel=None, master=None):
    ...
    def pausu_bat(self):
        ...
        try:
            ...
            try:
                ...
                self.amaitu.configure(state=ACTIVE)
    ...
    def jolastu(self):
        ...
        self.amaitu.configure(state=DISABLED)
        ...
        if Kontroladorea().musikaLortu(self.erab) != "default":
            pygame.mixer.init()
            if(Kontroladorea().musikaLortu(self.erab) == 'Soundtrack 1: Ballad of Godness'):
        ...
        self.gorde.configure(state=ACTIVE, command=lambda: Kontroladorea().gordePartida(self.tab,self.erab))
